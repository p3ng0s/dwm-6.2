...
enum { SchemeNorm, SchemeSel, SchemeWarn, SchemeUrgent, SchemeCustom, SchemeOk }; /* color schemes */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		do {
			...
			if (i> LENGTH(tags) / 2 && !(occ & 1 << i || m->tagset[m->seltags] & 1 << i))
				continue;
			x += TEXTW(tags[i]);
...
drawbar(Monitor *m)
{
	...
	for (i = 0; i < LENGTH(tags); i++) {
		...
		if (occ & 1 << i) {
			drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
			drw_rect(drw, x + boxw, user_bh - boxw, w - ( 2 * boxw + 1), boxw,
